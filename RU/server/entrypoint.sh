...
set -e
F=/flags
say(){ echo "Пароль для level$1:"; cat "$F/level$1"; }

# ===== SCP (level0): scpuser заперт в chroot, видит ТОЛЬКО level1.txt =====
...
mkdir -p /srv/www/level17; { echo "Склей куски по порядку."; say 18; } > /tmp/blob17; split -b 24 -d /tmp/blob17 /srv/www/level17/part.
say 20 > /srv/socatflag

# ===== Ч4: сжатие и хитрые кодировки =====
...
# level26 — basenc многослойка: base64url -> base16 -> base32
mkdir -p /srv/www/level26; say 27 | basenc --base64url -w0 | basenc --base16 -w0 | basenc --base32 -w0 > /srv/www/level26/next.enc
# level27 — октальные escape-последовательности (\NNN), разворот через printf
...
echo "[fileserver] up (parts 1-5, levels 0-33)"
wait
